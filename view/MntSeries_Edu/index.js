...
var idDatatables = "series_table"; //TODO: CAMBIAR EL STRING SEGUN EL MANTENIMIENTO
var phpPrincipal = "series_Edu.php"; //TODO: CAMBIAR EL STRING SEGUN EL MANTENIMIENTO
var idModalAgregar = "insertar-serie-modal"; //TODO: CAMBIAR EL STRING SEGUN EL MANTENIMIENTO
...
columnDefs: [
    ...
    //nombre
    { targets: [1], orderData: [1], visible: true, width: "30%", className: 'tx-bold' },
    //valor
    { targets: [2], orderData: [2], visible: true, width: "8%" },
    //valor
 ...
 "ajax": {
        ...
        url: "../../controller/"+phpPrincipal+"?op=listarSeries",
        type: "get",
        dataType: "json",
        cache: false,
        serverSide: true,
...
function agregarElemento() {
  ...
  var formData = new FormData($("#insertar-serie-form")[0]);
  ...
  if (!validacion) {
    ...
    $.ajax({
      url: "../../controller/series_Edu.php?op=insertarSerie",
      type: "POST",
      data: formData,
      contentType: false,
      processData: false,
      success: function () {
          $('#series_table').DataTable().ajax.reload();

          toastr.success("Serie añadida");
          $('#insertar-serie-modal').modal('hide');
      }
  });
  } else {
...
function cargarElemento(idElemento) {
  ...
  $("#idPagoModal").val(idElemento);
  $.ajax({   
    url: '../../controller/pagos_Edu.php?op=recogerPago&id=' + idElemento,
    type: 'GET',
    dataType: 'json',
    cache: false,
    serverSide: true,
...
function editarElemento() {
  ...
  if (!validacion) {
    ...
    $.ajax({
      url: "../../controller/pagos_Edu.php?op=editarPago",
      type: "POST",
      data: formData,
      contentType: false,
      processData: false,
      success: function () {
        $('#pagos_table').DataTable().ajax.reload(null, false);

  
          toastr.success("El medio de pago se ha editado");
...
function eliminar(idSerie) {
  swal.fire({
      ...
      if (result.isConfirmed) {
          ...
          $.post("../../controller/series_Edu.php?op=eliminar", { idSerie: idSerie }, function (data) {

              $('#series_table').DataTable().ajax.reload();
  
              swal.fire(
                  'Serie',
                  'La Serie se he eliminado',
                  'success'
              )
          });
      }
