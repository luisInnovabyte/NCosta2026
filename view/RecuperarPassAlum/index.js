...
btnEnviarConfirmacion.click(function() {
    let nicknameRecu = $('#nicknameRecu').val();

    $.post("../../controller/usuario.php?op=comprobarNicknamePassword",{nickname:nicknameRecu},function (data) {

        if(data != 0){ // SI DA 1, SIGNIFICA QUE HAY DATOS = HAY CORREO CREADO
            console.log(data);
          $('#email_recuperar').val(data);  
          actualizar(data); 

        }else{
            toastr["error"]('El nickname proporcionado no se encuentra registrado.');
        }
    });
});
...
btnConfirmarCodigo.click(function() {
    ...
    var nickname = $("#nicknameRecu").val();
    console.log(nickname);
    if (validado === true) {
        $.post("../../controller/usuario.php?op=getTokenByNick",{nickname:nickname},function (data) {
