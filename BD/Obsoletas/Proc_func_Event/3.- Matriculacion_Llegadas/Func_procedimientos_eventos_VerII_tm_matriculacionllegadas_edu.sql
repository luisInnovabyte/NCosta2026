...
DELIMITER $$ CREATE FUNCTION estado_matriculacion(id_llegada_param INT) RETURNS INT DETERMINISTIC BEGIN
DECLARE resultado INT DEFAULT 0;
-- Verificar si LA LLEGADA no tiene matriculaciones registradas
IF (
    SELECT COUNT(*)
    FROM tm_matriculacionllegadas_edu
    WHERE idLlegada_matriculacion = id_llegada_param
) = 0 THEN
SET resultado = 4;
-- No tiene matriculaciones, devolver -1, para el caso de que solo tenga alojamiento y/o visado.
## 4 - En la linea y el cabecera significa que no tiene matriculaciones
ELSE ## SI TODAS HAN FINALIZADO
IF (
    SELECT COUNT(*)
    FROM tm_matriculacionllegadas_edu
    WHERE idLlegada_matriculacion = id_llegada_param
        AND DATE(fechaFinMatriculacion) < CURDATE()
) = (
    SELECT COUNT(*)
    FROM tm_matriculacionllegadas_edu
    WHERE idLlegada_matriculacion = id_llegada_param
...
SET resultado = 3;
-- Todas las matriculaciones han finalizado
ELSEIF (
    SELECT COUNT(*)
    FROM tm_matriculacionllegadas_edu
    WHERE idLlegada_matriculacion = id_llegada_param
        AND DATE(fechaInicioMatriculacion) > CURDATE()
) = (
    SELECT COUNT(*)
    FROM tm_matriculacionllegadas_edu
    WHERE idLlegada_matriculacion = id_llegada_param
...
SET resultado = 2;
-- Todas las matriculaciones aún no han iniciado
ELSEIF (
    SELECT COUNT(*)
    FROM tm_matriculacionllegadas_edu
    WHERE idLlegada_matriculacion = id_llegada_param
        AND CURDATE() BETWEEN DATE(fechaInicioMatriculacion) AND DATE(fechaFinMatriculacion)
) > 0 THEN
SET resultado = 1;
-- Hay al menos una matrícula activa
END IF;
END IF;
RETURN resultado;
END $$ DELIMITER;
...
#########################################################################################################
DELIMITER $$ CREATE PROCEDURE actualizar_estMatricula() BEGIN -- Declarar variables necesarias
...
-- Abrir el cursor
OPEN cur;
read_loop: LOOP -- Leer el siguiente registro
...
##    PARA EL PROGRAMA                                                                              #####
#########################################################################################################
DELIMITER $$ CREATE PROCEDURE actualizar_estMatriculaxidLlegada(IN idLlegada_param INT) BEGIN -- Declarar variables necesarias
## Esta marca el final del bucle
DECLARE done INT DEFAULT 0;
DECLARE v_id_llegada INT;
-- Declarar un cursor para recorrer los registros que no estén cancelados
...
############################################################################
DELIMITER $$ CREATE PROCEDURE actualizar_estLlegada_matriculacion() BEGIN ## pongo el estado de las matriculaciones a 3 cuando la fecha actual es superior a la fecha de finalización (ya ha finalizado el curso).
...
where idLlegada_matriculacion in (
        select id_llegada
        from tm_llegadas_edu
        where estLlegada = 0
    );
...
############################################################################
DELIMITER $$ CREATE PROCEDURE actualizar_estLlegada_matriculacion_xidllegada(IN idLlegada_matriculacion_param INT) BEGIN -- Pongo el estado de las matriculaciones a 3 cuando la fecha actual es superior a la fecha de finalización (ya hafinalizado el curso)
UPDATE tm_matriculacionllegadas_edu
SET estMatriculacion_llegadas = 3
WHERE DATE(fechaFinMatriculacion) < CURDATE()
...
########################################################################
DELIMITER $$ CREATE EVENT actualizar_estLlegada_evento_12_01 ON SCHEDULE EVERY 1 DAY STARTS CURRENT_DATE + INTERVAL 1 MINUTE DO # POR SI HACE FALTA METER MAS DE UNA LINEA
BEGIN CALL actualizar_estMatricula();
...
########################################################################
DELIMITER $$ CREATE EVENT actualizar_estLlegada_evento_08_00 ON SCHEDULE EVERY 1 DAY STARTS CURRENT_DATE + INTERVAL 8 HOUR DO # POR SI HACE FALTA METER MAS DE UNA LINEA
BEGIN CALL actualizar_estMatricula();
CALL actualizar_estLlegada_matriculacion();
CALL actualizar_estAlojamiento();
CALL actualizar_estLlegada_alojamiento();
...
########################################################################
DELIMITER $$ CREATE EVENT actualizar_estLlegada_evento_16_00 ON SCHEDULE EVERY 1 DAY STARTS CURRENT_DATE + INTERVAL 16 HOUR DO # POR SI HACE FALTA METER MAS DE UNA LINEA
BEGIN CALL actualizar_estMatricula();
CALL actualizar_estLlegada_matriculacion();
CALL actualizar_estAlojamiento();
CALL actualizar_estLlegada_alojamiento();
