...
SELECT 
    ...
    -- Total de matriculaciones (con IVA y descuento aplicados)
    -- Fórmula: Subtotal = Precio - (Precio * Descuento/100)
    --          Total = Subtotal + (Subtotal * IVA/100)
    COALESCE((
        SELECT SUM(
            -- Paso 1: Calcular subtotal con descuento
            (CAST(REPLACE(REPLACE(REPLACE(m.precioTarifa_matriculacion, '€', ''), '.', ''), ',', '.') AS DECIMAL(10,2)) 
             - (CAST(REPLACE(REPLACE(REPLACE(m.precioTarifa_matriculacion, '€', ''), '.', ''), ',', '.') AS DECIMAL(10,2)) 
                * COALESCE(m.descuento_matriculacion, 0) / 100))
            +
            -- Paso 2: Aplicar IVA sobre el subtotal
            ((CAST(REPLACE(REPLACE(REPLACE(m.precioTarifa_matriculacion, '€', ''), '.', ''), ',', '.') AS DECIMAL(10,2)) 
              - (CAST(REPLACE(REPLACE(REPLACE(m.precioTarifa_matriculacion, '€', ''), '.', ''), ',', '.') AS DECIMAL(10,2)) 
                 * COALESCE(m.descuento_matriculacion, 0) / 100))
             * COALESCE(m.idIvaTarifa_matriculacion, 0) / 100)
        )
    ...
    -- Total de alojamientos (con IVA y descuento aplicados)
    -- Fórmula: Subtotal = Precio - (Precio * Descuento/100)
    --          Total = Subtotal + (Subtotal * IVA/100)
    COALESCE((
        SELECT SUM(
            -- Paso 1: Calcular subtotal con descuento
            (CAST(REPLACE(REPLACE(REPLACE(al.precioTarifa_alojamientos, '€', ''), '.', ''), ',', '.') AS DECIMAL(10,2)) 
             - (CAST(REPLACE(REPLACE(REPLACE(al.precioTarifa_alojamientos, '€', ''), '.', ''), ',', '.') AS DECIMAL(10,2)) 
                * COALESCE(al.descuento_alojamientos, 0) / 100))
            +
            -- Paso 2: Aplicar IVA sobre el subtotal
            ((CAST(REPLACE(REPLACE(REPLACE(al.precioTarifa_alojamientos, '€', ''), '.', ''), ',', '.') AS DECIMAL(10,2)) 
              - (CAST(REPLACE(REPLACE(REPLACE(al.precioTarifa_alojamientos, '€', ''), '.', ''), ',', '.') AS DECIMAL(10,2)) 
                 * COALESCE(al.descuento_alojamientos, 0) / 100))
             * COALESCE(al.idIvaTarifa_alojamientos, 0) / 100)
        )
    ...
    (
        ...
        -- Total alojamientos con descuento e IVA
        COALESCE((
            SELECT SUM(
                (CAST(REPLACE(REPLACE(REPLACE(al.precioTarifa_alojamientos, '€', ''), '.', ''), ',', '.') AS DECIMAL(10,2)) 
                 - (CAST(REPLACE(REPLACE(REPLACE(al.precioTarifa_alojamientos, '€', ''), '.', ''), ',', '.') AS DECIMAL(10,2)) 
    ...
    -- PAGO PENDIENTE = Total General (con descuentos e IVA) - Pagos Realizados
    (
        (
            -- Total matriculaciones con descuento e IVA
            COALESCE((
    ...
    -- Porcentaje de pago realizado (con descuentos e IVA aplicados en el denominador)
    CASE 
        WHEN (
