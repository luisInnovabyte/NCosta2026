...
case "mostrarAlumnosFinal":
   ...
   // SI SE TIENE TANTO IDDEPARTAMENTO, FECHA INICIO Y FECHA FIN, SE FILTRA TANTO POR DEPARTAMENTO, COMO POR FECHAS
    ...
    // Función para formatear fechas concatenadas separadas por ' | '
    $formatFecha = function($fechas) {
        ...
        return implode(' | ', $fechasFormateadas);
    };

    // Función para formatear fechas simples yyyy-mm-dd a dd-mm-yyyy
    $formatFechaSimple = function($fecha) {
    ...
    // Aplicar formato a los campos de fechas antes de armar el array de salida
    $fechasInicioMatricula = $formatFecha($row["fechas_inicio_matricula"]);
    $fechasFinMatricula = $formatFecha($row["fechas_fin_matricula"]);
    $fechaInicioMasTemprana = $formatFechaSimple($row["fecha_inicio_mas_temprana"]);
    $fechaFinMasTardia = $formatFechaSimple($row["fecha_fin_mas_tardia"]);
    ...
    $sub_array[] = $fechasInicioMatricula;
    $sub_array[] = $fechasFinMatricula;
    $sub_array[] = $fechaInicioMasTemprana;
    $sub_array[] = $fechaFinMasTardia;
...
case "mostrarObjetivosPorGrupo":
    ...
    case "obtenerDetallesCursoAlumno":
        ...
        $file = 'aaListarCursosAlumnoParametros.json';
        ...
        $minutosCertificado = $_POST['minutosCertificado'];
        $jornadaClase = $_POST['jornadaClase'];
        $textoDescripcion = isset($_POST['textoDescripcion']) ? $_POST['textoDescripcion'] : null;

        $datosAlumnos = $evaluacionFinal->insertarEvaluacion($mostrarCertificado, $resultadoEvaluacion, $idLlegada, $tokenUsu, $codigoGrupo, $modo, $minutosCertificado, $jornadaClase, $textoDescripcion);
        ...
        $minutosCertificado = isset($_POST['minutosCertificado']) ? $_POST['minutosCertificado'] : null;
        $jornadaClase = isset($_POST['jornadaClase']) ? $_POST['jornadaClase'] : null;
        $textoDescripcion = isset($_POST['textoDescripcion']) ? $_POST['textoDescripcion'] : null;

        $datosAlumnos = $evaluacionFinal->editarEvaluacion($idCertificado, $mostrarCertificado, $resultadoEvaluacion, $idLlegada, $tokenUsu, $codigoGrupo, $modo, $minutosCertificado, $jornadaClase, $textoDescripcion);

        echo json_encode($datosAlumnos);
        break;

        case "desactivarCursos":
            $idLlegada = $_POST['idLlegada'] ?? null;
            if ($idLlegada) {
                $resultado = $evaluacionFinal->cambiarEstadoLlegadaCurso($idLlegada);
                echo json_encode(['success' => $resultado]);
            } else {
                echo json_encode(['success' => false, 'error' => 'Falta idLlegada']);
            }
            break;
