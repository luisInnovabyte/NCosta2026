...
switch ($_GET["op"]) {
    ...
    case "cargarDatosEditar":
        ...
        $datosActividad = $actividad->getActividad_x_id($_POST['idAct']);
    ...
    case "mostrarListaAlum":
        ...
        echo json_encode($listaAlum);

        break;
        
    case "insertarAlumnoActividad":

    $resultado = $actividad->insertarAlumnoActividad($_POST['idAct'], $_POST['selectAlumno']);

    session_start();
    require_once("../models/Log.php");

    if ($resultado) {
        // Archivo LOG solo si se ha insertado
        $nombreLog =  $_SESSION['usu_id'] . " - " . $_SESSION['usu_nom'];
        $logI = new Log($nombreLog, "actividades.php",  "Alumno " . $_POST['selectAlumno'] .  " fue apuntado por " .   $_SESSION['usu_nom'] .  " a la actividad " . $_POST['idAct']);
        $logI->grabarLinea();
        unset($logI);
...
case "mostrarActUsuarioPerfil":
    ...
    case "apuntarse":
        $idUsu = $_POST["idUsu"];
        $idAct = $_POST["idAct"];
        $actividad->apuntarse($idUsu, $idAct);
        session_start();
        require_once("../models/Log.php");
    ...
    case "mostrarAlumno":
        ...
        $datos = $actividad->mostrarAlum($_GET['idAct']);
        session_start();
        require_once("../models/Log.php");

        // Archivo LOG
        $nombreLog =  $_SESSION['usu_id'] . " - " . $_SESSION['usu_nom'];
        $logI = new Log($nombreLog, "actividades.php",  "Lista los alumnos apuntados en la actividad " . $_GET["idAct"]);
        $logI->grabarLinea();
        unset($logI);
        // FIN del archivo LOG
        $data = array();
        ...
        $results = array(
            ...
            "iTotalDisplayRecords" => count($data),
            "aaData" => $data,
            "extraData" => $extraVariable  // Aquí agregas tu nueva variable
