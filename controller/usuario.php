...
switch ($op) {
    ...
    case "comprobarNickname":
        $nickUsu = strtolower(trim($_POST['nickname'])); // Poner en minuscula

        $datosUsu = $usuario->get_usuario_x_nick(strtolower(trim($nickUsu)));
        if (is_array($datosUsu) == true and count($datosUsu) > 0) {  // si es mayor que cero es que ya existe uno y debo sacar error
            echo 1;
        };
    break;
    case "comprobarNicknamePassword":
        $nickUsu = strtolower(trim($_POST['nickname'])); // Poner en minuscula

        $datosUsu = $usuario->get_usuario_x_nick(strtolower(trim($nickUsu)));
        if (is_array($datosUsu) == true and count($datosUsu) > 0) {  // si es mayor que cero es que ya existe uno y debo sacar error
            echo $datosUsu[0]['correoUsu'];
        }else{
            echo 0;
        };
    break;
    ...
    case "registrarUsuario":
        ...
        $nickName =  sanearLogin($_POST['nickname']); //NickName
    ...
    case "loginAlum":
        ...
        $datos = $usuario->loginAlum($nickname, $usu_pass);
          //Archivos Log
          generarLog('usuario.php', 'Se hace login Alumno'); //EJEMPLO: generarLog('empresa.php','Listar datos de empresa');
          //FIN Log
        echo $datos;
...
<button type="button" onClick=""  id="' . $row["idUsu"] . '" class="btn btn-danger btn-icon mt-1 mt-lg-0 disabled" data-toggle="tooltip-primary" data-placement="top" title="Los administradores no pueden ser desactivados."><div><i class="fa-solid fa-user-slash"></i></div></button>';
    ...
    case "getTokenByCorreo":
        ...
        case "getTokenByNick":
            ...
            $datosUsuario = $usuario->get_token_x_nickname($_POST["nickname"]);
    ...
    case "guardardatosUsuarioConductor":
        ...
        $resultado = $usuario->guardardatosUsuarioConductor($tokenUsu, $nombreUsu, $apellidosUsu, $movilUsu, $correoUsu, $ciudadPuebloUsu, $codigoPostalUsu, $fechaNacimientoUsu, $signatureDataReceptor);
