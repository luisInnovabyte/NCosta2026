...
class Usuario extends Conectar
{
    ...
    public function get_personal_x_usu($correoUsu)
    {
        $conectar = parent::conexion();
        parent::set_names();

        $sql = "SELECT * FROM tm_personal WHERE trim(emailUsuario) = trim('$correoUsu')";
        
        $sql = $conectar->prepare($sql);
        $sql->execute();
        return $resultado = $sql->fetchAll();
    }
    public function get_usuario_x_nick($nickUsu)
    {
        $conectar = parent::conexion();
        parent::set_names();

        $sql = $sql = "SELECT * FROM tm_usuario WHERE LOWER(TRIM(nickUsu)) = LOWER(TRIM('$nickUsu'))";
    ...
    public function guardardatosUsuarioConductor($tokenUsu,$nombreUsu,$apellidosUsu,$movilUsu,$correoUsu,$ciudadPuebloUsu,$codigoPostalUsu,$fechaNacimientoUsu,$signatureDataReceptor)
    {
        ...
        return $resultado3 = $sql->fetchAll();
    ...
    public function getUsuario_x_idTokenView($idToken)
    {
     
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "SELECT * FROM tm_alumno_edu LEFT JOIN tm_usuario ON tm_alumno_edu.idUsuario_tmalumno = tm_usuario.idUsu LEFT JOIN tm_prescriptores ON tm_alumno_edu.tokenUsu COLLATE utf8mb3_general_ci = tm_prescriptores.tokenPrescriptores COLLATE utf8mb3_general_ci WHERE  tm_alumno_edu.`tokenUsu` = '$idToken'";
    ...
    public function login($usu_correo, $usu_pass)
    {
        ...
        if (empty($correo) or empty($pass)) {
            ...
            $sql = "SELECT * FROM tm_usuario WHERE correoUsu = '$correo' AND senaUsu = MD5('$pass') AND rolUsu IN (1, 2);";
            ...
            if (is_array($resultado) and count($resultado) > 0) { // ESTA LOGEADO
                ...
                if ($resultado['estUsu'] == 1) {
                    ...
                    $_SESSION['usuPre_idInscripcion'] = $resultado['idInscripcion_tmusuario'];
    ...
    //LOGIN ALUM
    public function loginAlum($nickUsu, $usu_pass)
    {

        $conectar = parent::conexion();
        parent::set_names();

        $nickUsu = strtolower($nickUsu);
        $pass = $usu_pass;

        if (empty($nickUsu) or empty($pass)) {
            return 'Campos vacios';
        } else {


            $sql = "SELECT * FROM tm_usuario left join tm_prescriptores ON tm_usuario.idInscripcion_tmusuario = tm_prescriptores.idPrescripcion WHERE nickUsu = '$nickUsu' AND senaUsu = MD5('$pass')";
            ...
            if (is_array($resultado) and count($resultado) > 0) { // ESTA LOGEADO
                ...
                if ($resultado['estUsu'] == 1) {
                    ...
                    $_SESSION['usu_email'] = $resultado['correoUsu'];
                    $_SESSION['usu_rol'] = $resultado['rolUsu'];
                    $_SESSION['usu_nickname'] = $resultado['nickUsu'];
                    $_SESSION['usu_est'] = $resultado['estUsu'];
                    $_SESSION['usu_obsUsu'] = $resultado['obsUsu'];
                    $_SESSION['usu_avatar'] = $resultado['avatarUsu'];
                    $_SESSION['usu_genero'] = $resultado['generoUsu'];
                    ...
                    $_SESSION['registroUsu'] = $resultado['registroUsu'];
                    $_SESSION['usu_uuidUsu'] = $resultado['uuidUsu']; // UUID API
                    $_SESSION['usu_idInteresado'] = $resultado['idInscripcion_tmusuario']; // UUID API
                    $_SESSION['usu_idAlumno'] = $resultado['idAlumno_tmusuario']; // UUID API
                   /*  $_SESSION['usu_idTransportistaUsu'] = $resultado['idTransportista_transportistas-Transporte']; */ // TRANSPORTISTA

                    // VARIABLES DE INSCRIPCIONES
                    //$_SESSION['usuPre_departamento'] = $resultado['idDepartamentoEdu_prescriptores']; // DEPARTAMENTO
                    $_SESSION['usuPre_idInscripcion'] = $resultado['idInscripcion_tmusuario'];
 ...
 */
    ...
    public function get_token_x_nickname($nickUsu)
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "SELECT `tokenUsu` FROM tm_usuario WHERE `nickUsu` = '$nickUsu'";
    ...
    public function recogerIdxCorreo($correo){
        ...
        // se hace sobre una vista para sacar el literal de la categoria
        // $sql = "SELECT * FROM vsopcatego WHERE estSop=1";
        $sql = "SELECT `idAlumno` FROM `tm_alumno_edu` WHERE emailUsuario = '$correo'";
    ...
    public function crearToken($idUsu,$token){
        ...
        // se hace sobre una vista para sacar el literal de la categoria
        // $sql = "SELECT * FROM vsopcatego WHERE estSop=1";
        $sql = "UPDATE tm_alumno_edu SET tokenUsu=$token WHERE idAlumno = '$idUsu'";
    ...
    public function getUserByToken($token){
        ...
        // se hace sobre una vista para sacar el literal de la categoria
        // $sql = "SELECT * FROM vsopcatego WHERE estSop=1";
        $sql = "SELECT * FROM tm_alumno_edu WHERE tokenUsu = '$token'";
    ...
    public function actualizarConocimientosPerfil($idUsuario, $lenMatAlumno, $lenCon1Alumno, $lenCon2Alumno, $lenCon3Alumno, $lenCon4Alumno, $estEspAlumno, $nivEspAlumno, $tiemEspAlumno, $lugEspAlumno, $porEspAlumno, $mejEspAlumno)
    {
        ...
        // se hace sobre una vista para sacar el literal de la categoria
        // $sql = "SELECT * FROM vsopcatego WHERE estSop=1";
        $sql = "UPDATE tm_alumno_edu  SET lenMatAlumno = '$lenMatAlumno', 
    ...
    public function actualizarObjetivosPerfil($idUsuario, $gus1EspAlumno, $gus2EspAlumno, $gus3EspAlumno, $gus4EspAlumno, $gus5EspAlumno, $gusTextEspAlumno, $conAlumno, $conRecoAlumno, $conAgenAlumno)
    {
        ...
        $sql = "UPDATE tm_alumno_edu SET gus1EspAlumno = '$gus1EspAlumno', 
    ...
    //###################################################///
    //######## CONFIGURACIÓN DE ADAPTACIONES  ############//
    ///###################################################///
    public function actualizarAdaptacionesPerfil($idUsuario, $agoraAlumno, $minusvaliaAlumno, $obsMinusvaliaAlumno)
    {
        $conectar = parent::conexion();
        parent::set_names();
        // se hace sobre una vista para sacar el literal de la categoria
        // $sql = "SELECT * FROM vsopcatego WHERE estSop=1";
        $sql = "UPDATE `tm_alumno_edu` SET 
        `agoraAlumno` = '$agoraAlumno',
        `minusvaliaAlumno` = '$minusvaliaAlumno',
        `obsMinusvaliaAlumno` = '$obsMinusvaliaAlumno'
    ...
    public function editarAvatar($avatarUsuario, $idUsuario)
    {
        ...
        // se hace sobre una vista para sacar el literal de la categoria
        // $sql = "SELECT * FROM vsopcatego WHERE estSop=1";
        $sql = "UPDATE `tm_usuario` SET `avatarUsu`='$avatarUsuario' WHERE `tokenUsu` = '$idUsuario'";;
    ...
    public function cambiarBloqueado($idUsu){
        ...
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "UPDATE `tm_alumno_edu` SET `perfilBloqueado` = NOT `perfilBloqueado` WHERE idAlumno= $idUsu" ;
        $sql = $conectar->prepare($sql);
        $sql->execute();
        return $resultado = $sql->fetchAll();
    }
    public function recogerBloqueado($idUsu){
        ...
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "SELECT `perfilBloqueado` FROM `tm_alumno_edu` WHERE idAlumno= $idUsu" ;
        $sql = $conectar->prepare($sql);
        $sql->execute();
        return $resultado = $sql->fetchAll();
    }
