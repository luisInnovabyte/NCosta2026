...
<link href="../../public/lib/summernote/summernote.css">
...
<script>
    ...
    // Función para calcular la diferencia en días entre una fecha pasada y la fecha actual
    // Esta función toma una fecha pasada en formato "YYYY-MM-DD HH:mm:ss" y devuelve la cantidad de días transcurridos desde esa fecha hasta la fecha actual.
    function calcularDesdeFecha(fechaPasada) {
        // Obtener la fecha actual
        const fechaActual = new Date();

        // Convertir la fecha pasada a objeto Date
        const fechaPasadaDate = new Date(fechaPasada);

        // Verificar si la fecha pasada es válida
        if (isNaN(fechaPasadaDate.getTime())) {
            return NaN; // Devolver NaN si la fecha es inválida
        }

        // Calcular la diferencia en milisegundos
        const diferenciaEnMilisegundos = fechaActual.getTime() - fechaPasadaDate.getTime();

        // Calcular la diferencia en días
        const diferenciaEnDias = Math.floor(diferenciaEnMilisegundos / (1000 * 60 * 60 * 24));

        // Calcular las horas restantes
        const diferenciaEnHoras = Math.floor((diferenciaEnMilisegundos % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));

        // Devolver la diferencia en días y horas
        return `${diferenciaEnDias} días y ${diferenciaEnHoras} horas`;
    }

    // Ejemplo de uso:
    // const fechaEjemplo = "2024-01-20 10:30:00";
    // const diasPasados = calcularDiasDesdeFecha(fechaEjemplo);
    // console.log(`Días pasados desde ${fechaEjemplo}: ${diasPasados}`);


    // Función para extraer el número de días de un texto
    // Esta función toma un texto que contiene la palabra "días" y devuelve el número de días como un entero.
    function calcularDiasDesdeFecha(fechaPasada) {
        // Obtener la fecha actual
        const fechaActual = new Date();
